      ...
      >
        ...
        >
          <div class="relative flex-shrink-0">
            <div class="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center border border-indigo-100">
              <Icon name="lucide:user" class="w-5 h-5 text-indigo-600 stroke-[1]" />
            </div>
            <div class="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></div>
          </div>
          
          <div v-if="!isCollapsed" class="flex flex-col items-start overflow-hidden">
            <span class="text-xs font-normal text-neutral-800 truncate w-full tracking-tight">{{ userProfile?.nome || 'Usuário' }}</span>
          ...
          <Icon 
            ...
            class="w-3.5 h-3.5 text-neutral-300 ml-auto stroke-[1]" 
          />
        </button>
        ...
        >
          ...
          >
            <NuxtLink 
              to="/perfil" 
              class="flex items-center gap-3 px-3 py-2.5 text-sm font-normal text-neutral-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all"
              @click="isUserMenuOpen = false"
            >
              <Icon name="lucide:user-cog" class="w-4 h-4 stroke-[1]" />
              <span>Meu Perfil</span>
            </NuxtLink>
            <button 
              @click="handleLogout"
              class="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-normal text-red-500 hover:bg-red-50 rounded-xl transition-all mt-1"
            >
              <Icon name="lucide:log-out" class="w-4 h-4 stroke-[1]" />
